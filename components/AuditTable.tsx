...
const AuditRowItem = React.memo(({ 
  ...
  <tr className={idx % 2 === 0 ? 'bg-white hover:bg-slate-50' : 'bg-slate-50 hover:bg-blue-50 transition-colors'}>
    <td className={`${cellTextClass} text-left font-bold text-slate-900 py-4 px-3 md:px-4 uppercase`}>
      {formatDisplayName(row.corridor)}
    </td>
    {!isClassReport && (
      <td className={`${cellTextClass} text-left font-black text-slate-950 w-20 md:w-32 py-4 pl-4 pr-2 md:px-4 text-[11px] md:text-base`}>
        {row.sku.toLocaleString('pt-BR')}
      </td>
    )}
    <td className={`${cellTextClass} text-center font-black text-slate-950 w-20 md:w-32 py-4 px-2 text-[11px] md:text-base`}>
      {row.notRead.toLocaleString('pt-BR')}
    </td>
    {!isClassReport && (
      <td className={`${cellTextClass} text-center w-28 md:w-40 py-4 px-2 md:px-4`}>
...
const AuditTable: React.FC<AuditTableProps> = ({ 
  ...
  const headerTextClass = 'text-[9px] md:text-sm py-4 px-2 md:px-4 font-black uppercase tracking-widest select-none text-black';
      ...
      <table className="w-full divide-y divide-slate-200 table-auto">
        <thead className="bg-slate-200">
          <tr>
            <th 
              scope="col" 
              className={`${headerTextClass} text-left cursor-pointer hover:bg-slate-300 transition-colors`}
            ...
            {!isClassReport && (
              <th 
                scope="col" 
                className={`${headerTextClass} text-left cursor-pointer hover:bg-slate-300 transition-colors w-20 md:w-32 pl-4 pr-2 md:px-4`}
            ...
            <th 
              scope="col" 
              className={`${headerTextClass} text-center cursor-pointer hover:bg-slate-300 transition-colors w-20 md:w-32`}
              onClick={() => !simpleView && handleSort('notRead')}
            >
               <div className="flex flex-col items-center justify-center leading-none">
                <div className="flex items-center">
                  <span className="md:hidden">NÃO</span>
                  <span className="hidden md:inline">NÃO LIDOS</span>
                ...
                <span className="md:hidden mt-0.5">LIDOS</span>
              </div>
            </th>
            {!isClassReport && (
              <th 
                scope="col" 
                className={`${headerTextClass} text-center cursor-pointer hover:bg-slate-300 transition-colors w-28 md:w-40`}
              ...
              >
                <div className="flex flex-col items-center justify-center leading-none">
                  ...
                  <span className="text-[11px] md:text-base mt-1">%</span>
                </div>
              </th>
            )}
          </tr>
